#!/usr/bin/env python3
"""Stop one strategy's worker — verify real ownership of its PID file, then signal it.
...
worker's own ``shutdown_signals`` handler turns that into an orderly
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--strategy-id", required=True, help="Which strategy's worker to stop.")
