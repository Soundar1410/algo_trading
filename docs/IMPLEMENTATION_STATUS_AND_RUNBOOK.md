...
### Third real strategy: `ema_cross_5_21_buy` — 29 August 2026

Added `ema_cross_5_21_buy`, a faithful clone of `ema_cross_9_21_buy` (Phase 9's
first real strategy) differing only in the fast EMA period (5 instead of 9)
and identity — the slow period (21) is unchanged. Same ATM-weekly BUY-only
single-position design, same `momentum_low_or_highest_close` premium exit (4%
activation / 8% trail), same 3% daily live-MTM cap on its own independent
₹10,00,000 capital base, same 09:15/14:45/15:15 timing, same
`minimum_separation: 0` / `confirmation_candles: 1` (no whipsaw damping —
operator decision). The one behavioural consequence of the period change:
none for the cold-start readiness gate — because the slow leg is unchanged
at 21, the gate stays **21 bars**, unlike the `ema_cross_5_9_buy` clone
(whose slow leg also changed, dropping its gate to 9 bars).

- **Shipped `enabled: false`** (operator decision) — built, tested and
  discoverable (`discover_strategies`), but does not run on the shared
  `intraday_options` feed alongside `ema_cross_9_21_buy` or
  `ema_cross_5_9_buy` until deliberately enabled. Runnable in isolation via
  the runtime's `--strategy-id ema_cross_5_21_buy` selector.
  `discover_enabled_strategies` correctly excludes it while disabled —
  verified against the real `config/` tree, not assumed.
- **No shared/combined risk cap.** If ever enabled, `ema_cross_5_21_buy` runs
  with its own independent ₹10,00,000 capital base and ₹30,000 daily-MTM cap,
  alongside `ema_cross_9_21_buy`'s and `ema_cross_5_9_buy`'s — no combined cap
  exists or was built; this is deliberately unbuilt, out of scope for this
  addition.
- **Wiring required no code changes** — config discovery
  (`common.config.loader.discover_enabled_strategies` globs
  `config/strategies/*.yaml`), the dashboard (renders per discovered
  ...
  (`@register_strategy` + dotted `strategy_ref`) are all already
  strategy-agnostic.
- **Files added:** `strategies/intraday_options/ema_cross_5_21_buy/`
  (`__init__.py`, `strategy.py`, `ema_cross_5_21_buy_spec.md`);
  `config/strategies/ema_cross_5_21_buy.yaml`;
  `tests/unit/test_ema_cross_5_21_buy_strategy.py`;
  `tests/integration/test_ema_cross_5_21_buy_engine.py`.
- **No change to `ema_cross_9_21_buy`'s or `ema_cross_5_9_buy`'s behaviour,
  code, or config values.** A repo-wide check for lingering "only real
  strategy" wording beyond what the 27 August 2026 `ema_cross_5_9_buy` change
  already softened found none remaining in either sibling strategy's files;
  no further comment edits were needed there. Full suite, ruff, and mypy all
  pass.
- CLAUDE.md's "do not implement real strategies beyond `ema_cross_9_21_buy`"
  line is stale as of Phase 9 (three real strategies now exist) and was
  flagged to the operator again; not edited here (operator's own file, per
  project rules).
...
### Two externally-reported defects, and the audit that cleared 62 more — 20 September 2026

An external test run on Linux (Python 3.11.15, installed from
`requirements.lock`) reported two defects. **Both were already fixed on
`HEAD`**, on the same day, five weeks before the report — the run was made
against a checkout older than 13 August 2026. See **D86** and **D87** for the
full evidence. What this entry records is the work that was genuinely
outstanding: the missing regression test, and a time-bomb audit that turned out
to explain almost every failure this runbook had been carrying as an accepted
baseline.

**Bug 1 — fork/spawn queue context.** `RuntimeError: A SemLock created in a
fork context is being shared with a process in a spawn context.`, 22 failures
across three end-to-end files on Linux, none on macOS. The production line was
corrected in `176d0c4` (13 August); the line numbers in the report match the
pre-fix file exactly. Nothing guarded it, though, and on macOS
`mp.get_context("spawn")` reads as redundant — so a revert would be invisible
locally. `tests/unit/test_bounded_worker_queue_start_method.py` now forces
`fork` in a child interpreter and hands a production-built queue to a spawned
process. Fail-first was proven by reverting that one line, watching the new
test fail with the exact reported error, then restoring it byte-identical. An
audit of every multiprocessing primitive in `runtimes/`, `common/`, `scripts/`
and `ui/` found zero further cases.

**Bug 2 — a stale scrip-master fixture.** Also already fixed (`3de9602`). The
audit it required is what mattered. `runtimes/positional_options/worker.py`
builds a bare `DhanOptionChainResolver(scrip_master)` just to read `lot_size`,
and that resolves `nearest_expiry()` against the **real** clock — inside a
spawned worker, out of reach of any `monkeypatch` — while the strategy's own
dated lookups stay on the simulated 2026-08-19 timeline. Six fixture masters
listed only a 2026-08-26 (or 2026-08-24) expiry, so from late August they read
as stale to one caller and correct to the other. Each now lists a second,
never-traded sentinel series **computed from the clock**, strictly after the
traded expiry: every dated lookup is unchanged, and the sentinel's distinct
security ids make a wrong lookup fail loudly. A literal future date was
rejected as a fix — it only resets the bomb, which is precisely what
`_FIXTURE_EXPIRY = "2030-12-25"` was set up to do on 2030-12-26.

**What that cleared.** The 65 "pre-existing failures" recorded on 31 August and
8 September, across nine files, were all this one defect. So was the
`test_positional_runtime_weekly_staged_entry.py` hang — recorded then as "a
separate, real `multiprocessing` deadlock (confirmed via `lsof` ... zero CPU
for over an hour)" and deselected ever since. It was never a deadlock: the
spawned worker died during construction and the parent blocked forever waiting
for it. **It now passes in 4.26 s.**

| | before | after |
|---|---|---|
| failures | 65 | **0** |
| indefinite hangs | 1 | **0** |
| full suite | 3556 passed (1 file excluded) | **3622 passed, 0 failed, 18 skipped** |

The arithmetic accounts exactly: 3556 + 65 fixed + 1 previously-hanging = 3622.

**The last 3 were two stacked defects, both unmasked by the fix above.** All in
`tests/unit/test_dashboard_positional_real_data.py`, previously hidden behind
the `ScripMasterError` failing first — the same shape as D77/D78, where fixing
one defect is what made the next one visible. Neither is a time bomb.

*First:* alone among its four sibling suites this file never pinned
`volatility_gate: {method: displacement}`, so the post-2026-09 `method:
realized` default could never fill its 60-sample window from a handful of
scripted ticks, and no cycle ever entered. Pinned, the cycle and all four legs
are genuinely written — verified directly against the database before touching
anything else.

*Second:* `discover_strategy_options()` admits a strategy only via a config
entry or a healthy heartbeat. Its docstring records the deliberate removal of
the old "left a trace in the trade tables" third condition — which is precisely
what this fixture had been relying on — so the picker came back empty and the
page rendered "Select a strategy above to see its cycles." Closed by having the
fixture `record_heartbeat(health_state="RUNNING_PAPER")` after the engine run,
which is what a real paper worker does and keeps the fixture faithful to this
module's own stated premise: rows "the same shape a real paper run would
produce." `config_root=None` is deliberate here — the subject is real data read
through `run_bounded`, never config discovery — so the heartbeat is the branch
that has to exist. Date-independent by construction: `_HEALTHY_STATES` matches
on the `health_state` column alone and never compares a beat's age against the
clock, so this adds no new bomb.

**Verification.** `ruff check .` clean. `mypy` clean over 246 source files.
Full suite as tabled above — **3622 passed, 0 failed, 18 skipped**, nothing
excluded, no hang, and for the first time in this runbook's record no
pre-existing failure baseline to carry forward. `ruff format --check`
reports 168 files, **unchanged by this work**: the same 7 modified files were
already unformatted at `HEAD` (verified file by file against their `HEAD`
versions) and both new files are clean — this repository has never been
`ruff format` clean, and reformatting 168 files was out of scope.

**Not done here.** No production file was changed — `common/feed/queues.py` was
reverted and restored byte-identically during the fail-first proof and is
...
### 2. A test that depended on the host timezone (D89)

```
tests/unit/test_auto_start_gate.py::test_a_matching_offset_counts_even_when_the_zone_name_differs
assert gate.system_timezone_matches("Asia/Kolkata", at=_at(9, 0))
E   AssertionError: assert False
```

Reproduced locally in one command: `TZ=UTC .venv/bin/python -m pytest
tests/unit/test_auto_start_gate.py` → **1 failed, 16 passed**, at line 136.

The test patches `system_timezone_name()` to `"Asia/Calcutta"` precisely to force
the decision onto its *offset* fallback — and `gate.py:118` then read that offset
from `moment.astimezone()`, the real machine. It passed only because this Mac is set
to IST, the single value that makes the assertion true. The file's own docstring
already promised otherwise: "every case here is decided against an explicit,
timezone-aware `now` rather than the real clock."

**The gate's production behaviour is correct and was not changed.** Refusing to
auto-start when the Mac's clock is not the configured zone is the entire point:
`launchd`'s `StartCalendarInterval` fires in the Mac's local zone, so on a
mismatched machine the 09:00 trigger is not the configured 09:00.

The one host read moved into a seam beside the one the test already substitutes:
...
Chosen over forcing the process timezone with `TZ` + `time.tzset()`: that mutates
process-global state every other module in the same interpreter shares, is
POSIX-only, and needs careful teardown, whereas this makes *both* host-derived
inputs to a two-input decision reachable the same way — which is what the test
needed and never had. Behaviour is byte-identical; no gate decision changes.

Two cases were added while the seam was there, neither previously pinned: a
genuinely different host zone must fail **both** checks (without it, a
`system_timezone_matches` that ignored the offset entirely would satisfy the
original test), and a `utcoffset()` of `None` must not count as agreement. **The
rest of the file was audited for the same dependency and has none** — every other
case either passes `check_system_timezone=False` or patches
`system_timezone_matches` wholesale, and `test_the_env_TZ_wins_when_set` sets `TZ`
itself and only reads `os.environ`; the reproduction above naming one failure and
