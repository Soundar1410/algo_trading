"""``st05_supertrend_buy`` — NIFTY 5-minute SuperTrend(period 1, multiplier 0.5),
ATM weekly options, BUY-only, intraday.

A faithful clone of ``st12_supertrend_buy`` (formerly ``supertrend_buy_1_1p2``),
changing only the SuperTrend multiplier (1.2 -> 0.5) and this strategy's own
identity. Every other line — the SuperTrend construction, the exit wiring, the
warm-up spec, lifecycle hooks, snapshot/restore, and every property — is
byte-for-byte identical (docstring references to the multiplier/name aside).

Full functional/design spec: ``SUPERTREND_BUY_1_0P5_ALGO_TRADING_SPEC.md`` in this
directory. Read that, and ``st12_supertrend_buy``'s own spec, before touching this
file — they are the authoritative source of the behaviour here, not this
docstring.
...
deliberately **not** used as a parity source, here or in ``st12_supertrend_buy``. This
strategy's own parity source is the in-repo ``st12_supertrend_buy`` strategy, itself
extracted from the legacy ``supertrend_fast`` ``strategy.py`` / ``config/config.yaml``
...
#:
#: **This floor is operator-approved and is NOT derived from the multiplier.** Lowering
#: the multiplier from 1.2 to 0.5 changes only the ATR band width and therefore how
#: often the trend flips — it says nothing about how much continuous history is needed
#: to trust a flip once it happens. 75 stays 75.
...
def _pick(explicit: Any, params: dict[str, Any], key: str, default: Any) -> Any:
    ...
    as ``c921_ema_cross_buy`` and ``st12_supertrend_buy``.
    """
    return explicit if explicit is not None else params.get(key, default)


@register_strategy("st05_supertrend_buy")
class SupertrendBuy1x0p5Strategy(BaseStrategy):
    """NIFTY 5m SuperTrend(1, 0.5) fresh flip -> ATM weekly CE/PE, BUY-only."""

    name = "st05_supertrend_buy"
    #: For a UI that wants a human label; not read by the engine.
    display_name = "SuperTrend Buy 1/0.5 — NIFTY ATM Option"
    ...
    def __init__(
        ...
        *,
        ...
        # Spec section 6: this variant's own defaults. Changing either is a
        # strategy-spec change, not an implementation detail.
        period = int(_pick(supertrend_period, p, "supertrend_period", 1))
        multiplier = float(_pick(supertrend_multiplier, p, "supertrend_multiplier", 0.5))
        ...
        # backstop that never fires. Same choice c921_ema_cross_buy and
        # st12_supertrend_buy made.
