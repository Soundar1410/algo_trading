...
**Strategy ID:** `st12_supertrend_buy`  
...
Use `st12_supertrend_buy` instead of `supertrend_fast`.

Reasons:

- `fast` is ambiguous and becomes misleading if the period or multiplier is later configured differently.
- `st12_supertrend_buy` identifies the SuperTrend-driven BUY-only structure and its defining parameters: ATR period `1` and multiplier `1.2`.
...
- Strategy ID: `st12_supertrend_buy`
- Package: `strategies/intraday_options/st12_supertrend_buy/`
- Config: `config/strategies/st12_supertrend_buy.yaml`
...
`config/strategies/st12_supertrend_buy.yaml` therefore carries the verified NSE 2026
calendar, copied from `config/global.yaml`. **This is strategy-scoped**: it applies to
`st12_supertrend_buy` only, changes nothing for `c921_ema_cross_buy` or
...
- `strategies/intraday_options/st12_supertrend_buy/__init__.py`
- `strategies/intraday_options/st12_supertrend_buy/strategy.py`
- `config/strategies/st12_supertrend_buy.yaml`
