"""``ema_cross_5_21_buy`` — NIFTY 5-minute EMA(5)/EMA(21) crossover, ATM weekly
options, BUY-only, intraday. The third real strategy, a faithful clone of
``ema_cross_9_21_buy`` (Phase 9's first real strategy) with only the fast EMA
period (5 instead of 9) and identity changed; the slow period (21) is
unchanged.

Full functional/design spec: ``ema_cross_5_21_buy_spec.md`` in this
...
* **The daily 3% live-MTM loss cap.**
  ...
  (see ``config/strategies/ema_cross_5_21_buy.yaml``), not strategy code.
...
@register_strategy("ema_cross_5_21_buy")
