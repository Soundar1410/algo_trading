...
leg as the original. Phase 10 (live enablement) is a separate, later
decision, not part of this document's implementation scope — the design here
anticipates it (the live gate stays fail-closed throughout, §10) but does not
authorize building it.

The 5/21 EMA crossover is chosen because it is mechanically identical to the
already-reviewed 9/21 crossover — same detection, same exits, same risk
model, same 21-bar readiness gate — with only the fast period shortened, so
it is simple to reason about and easy to validate against a chart, and any
failure surfaced during forward testing is far more likely to be a pipeline
defect than a strategy-logic subtlety.

---

## 2. Strategy Summary

On NIFTY 5-minute closed candles, a **fresh** EMA(5)/EMA(21) crossover during
...
| Fast / slow EMA | 5 / 21 (on close) | yes |
...
Two stateful EMAs over candle **close**, periods 5 and 21, from
...
2. A valid entry is produced only when a **newly completed current-day closed
   candle** changes the EMA5/EMA21 relationship relative to that context to the
...
- If startup has **no usable historical context** — e.g. the warm-up manager is
  ...
  first live candle that makes the **21-EMA ready (≥ 21 bars, since `is_ready`
...
|---|---|
| No usable warm-up/context; first EMA-ready candle after 21 bars | Establish context only — **no entry** |
...
> as a fast structural stop, matching the sibling strategies' settled behaviour
> (§12.4).
...
risk:
  ...
  daily:
    capital_base: 1000000      # Rs 10,00,000 -- same base as the sibling strategies
...
| EMA(5), EMA(21) | `common.indicators.ema.EMA` | reuse |
...
4. If step 2 produces no usable relationship (no warm-up manager, or a
   ...
   21-EMA-ready candle establishes context only, never an entry.
...
timeframe — same as the sibling strategies.
...
- **Whipsaw exposure closer to 9/21 than to 5/9 (build spec §3 note):** 5/21
  shares 9/21's wide slow leg, so its crossover cadence is far closer to 9/21
  than to the jumpier 5/9 pair — the faster fast leg makes entries a touch
...
- **Cold-start / unavailable-context rule (§4.4):** no usable warm-up/context ⇒
  ...
  trusted as a real prior relationship.
...
- Candle sequences that force a crossover must be **re-derived** against the
  actual `EMA(5)`/`EMA(21)`/`ConfirmedCrossover` math, not copied from the
  9/21 or 5/9 fixtures — a faster fast leg against the same slow leg crosses
  on different closes than either sibling.
