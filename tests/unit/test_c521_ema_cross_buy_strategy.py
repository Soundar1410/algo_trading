"""Behaviour-level proofs for ``EmaCross5x21BuyStrategy`` — the third real
strategy, a faithful clone of ``EmaCross9x21BuyStrategy`` (Phase 9) with only
the fast EMA period (5 instead of 9) and identity changed; the slow period
(21) is unchanged.

Full spec: strategies/intraday_options/c521_ema_cross_buy/c521_ema_cross_buy_spec.md.
...
``tests/integration/test_c521_ema_cross_buy_engine.py`` instead.

Every EMA-dependent test below uses ``ema_fast=2, ema_slow=3`` (an explicit
constructor override) rather than the production 5/21 defaults, purely so a
crossover can be reached in a handful of candles; a dedicated test pins that
the *defaults* really are 5 and 21. Every numeric candle sequence here was
...
ema_slow=3`` override drives every numeric sequence below (not the 5/21
production defaults), the sequences themselves are identical to the sibling
``test_c921_ema_cross_buy_strategy.py``/``test_c509_ema_cross_buy_strategy.py``
suites' -- only the class/id and the period-specific assertions differ.
"""
...
def _close_trade(strategy: EmaCross5x21BuyStrategy, position: OpenPosition, exit_price: float,
...
# ------------------------------------------------------------- 1. registration
def test_strategy_registers_as_c521_ema_cross_buy():
    assert "c521_ema_cross_buy" in available_strategies()
    assert EmaCross5x21BuyStrategy.name == "c521_ema_cross_buy"
...
def test_get_strategy_by_name_builds_from_cfg_parameters():
    ...
    strategy = get_strategy("c521_ema_cross_buy", _Cfg())
    assert isinstance(strategy, EmaCross5x21BuyStrategy)
    assert strategy.quantity_lots == 5
    # Defaults still apply for everything not in cfg.parameters.
    assert strategy._ema_fast.period == 5
    assert strategy._ema_slow.period == 21


# ------------------------------------------------------------------ 2. EMA periods
def test_default_periods_are_5_and_21():
...
def test_ema_slow_must_be_greater_than_ema_fast():
    with pytest.raises(ValueError, match="ema_slow"):
        EmaCross5x21BuyStrategy(ema_fast=21, ema_slow=5)
    with pytest.raises(ValueError, match="ema_slow"):
        EmaCross5x21BuyStrategy(ema_fast=21, ema_slow=21)


# --------------------------------------------------------- 3. warm-up readiness
def test_no_entry_before_both_emas_are_ready():
    """Cold start: no signal at all until the slow EMA has seen `ema_slow` bars.
    The strategy's own readiness gate (`on_candle`'s `is_ready` check) is
    period-agnostic -- it fires off whatever `ema_slow` actually is, so this
    override (`ema_slow=3`) exercises exactly the same mechanism the
    production default of 21 relies on; see `test_default_periods_are_5_and_21`
    for the production default itself."""
    strategy = _strategy()
    strategy.reset()
...
#
# The most recently completed relationship survives
# on_warmup_complete(context_trusted=True) as crossover context -- it is not
# wiped a second time. An entry requires a genuine current-day closed
...
def test_context_bullish_continuation_does_not_produce_ce():
    ...
    # EMAs untouched, and the context itself is PRESERVED, not reset a
    # second time.
...
def test_context_bearish_first_live_candle_bullish_produces_ce_buy():
    ...
    immediately -- no continuation candles needed first."""
    strategy = _strategy()
    strategy.reset()
    for i, c in enumerate((100.0, 100.0, 100.0, 90.0, 80.0, 70.0)):
        strategy.on_candle(_candle(c), _ts(i))
...
# Mechanically identical to rows 1/2 above (ConfirmedCrossover has no notion
# of calendar days -- context is context, whatever its source).
...
def test_day_start_reset_clears_stale_crossover_state_but_not_ema():
    """The day-start reset() (stale in-memory state from a previous
    process/day) clears the crossover detector but never the EMAs
    (SESSION_SPANNING)."""
    strategy = _strategy()
    strategy.reset()
    for i, c in enumerate((100.0, 100.0, 100.0, 90.0, 80.0, 70.0)):
...
def test_only_on_candle_calls_advance_state_no_hidden_tick_evaluation():
    ...
    corresponds to a genuinely *closed* bar."""
    strategy = _strategy()
    strategy.reset()
    for i in range(5):
