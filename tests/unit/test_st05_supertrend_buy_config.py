"""The committed ``config/strategies/intraday_options/st05_supertrend_buy.yaml``,
read through the real loader and the real intraday config adapter.
...
from common.execution.correlation import STRATEGY_TOKEN_LENGTH, strategy_token
...
def test_the_committed_config_is_enabled_paper_and_not_live_approved(resolved):
    """Spec section 17's minimum. Enabled from the start (8 September 2026
    operator decision, made before implementation — see the dated addendum in
    docs/IMPLEMENTATION_STATUS_AND_RUNBOOK.md), unlike ``st12_supertrend_buy``'s
    original disabled-at-delivery shipment. The two flags that still keep live
    execution unreachable, regardless."""
...
def test_the_enabled_strategy_is_discovered_and_enabled(resolved):
    """``discover_strategies`` is what the dashboard and the supervisor's
    mode-transition exposure check read; ``discover_enabled_strategies`` is
    what decides that a worker is started at all. Both include this strategy."""
...
def test_enabling_this_strategy_did_not_change_other_strategies_flags():
    """No other strategy's committed flags moved as a side effect of adding this
    one — in particular ``st12_supertrend_buy``, whose id changed the same day
    this strategy was added but whose behaviour must be unaffected by either
    change. Deliberately membership checks, not an exhaustive equality against
    the full enabled set — that set is this file's business only for
    ...
    assert STRATEGY_ID in enabled
    assert {"c921_ema_cross_buy", "straddle_920", "st12_supertrend_buy"} <= enabled
    ...
    # st12_supertrend_buy specifically: same multiplier, same behaviour as before
    # its rename — this strategy's addition changed only its own id, not its rules.
    st12 = load_resolved_config(CONFIG_ROOT, RUNTIME_ID, "st12_supertrend_buy")
    assert st12.strategy.enabled is True
    assert st12.strategy.parameters["strategy_kwargs"]["supertrend_multiplier"] == 1.2
...
# ---------------------------------------------------- correlation-token safety
def test_every_committed_intraday_options_strategy_id_has_a_pairwise_distinct_token():
    """Spec §0.1/§2: the obvious id ``supertrend_buy_1_0p5`` collides with
    ``supertrend_buy_1_1p2`` on ``strategy_token()``'s 4-character truncation —
    the exact failure that forced the ``c921``/``c509``/``c521`` ema rename and
    then the ``st12``/``st05`` supertrend rename.

    Written generically against every strategy id **discovered from the real
    committed config tree**, not a hardcoded list — so this catches a *future*
    similarly-prefixed strategy colliding with something already admitted, not
    just re-proving today's set is fine. ``IntradayOptionsSupervisor.add_worker``
    is what actually enforces this at admission time; this test proves the
    committed set never reaches that refusal in the first place.
    """
    discovered = [c.strategy.strategy_id for c in discover_strategies(CONFIG_ROOT, RUNTIME_ID)]
    assert STRATEGY_ID in discovered
    assert len(discovered) >= 2, "nothing to prove collision-freedom against"

    tokens: dict[str, str] = {}
    collisions: dict[str, list[str]] = {}
    for strategy_id in discovered:
        token = strategy_token(strategy_id)
        assert len(token) <= STRATEGY_TOKEN_LENGTH
        if token in tokens.values():
            colliding_with = [sid for sid, t in tokens.items() if t == token]
            collisions.setdefault(token, colliding_with).append(strategy_id)
        tokens[strategy_id] = token

    assert collisions == {}, (
        f"strategy_token() collisions among committed intraday_options strategy ids: "
        f"{collisions!r} (tokens: {tokens!r})"
    )
...
def test_the_worker_requires_a_tick_channel(worker):
    ...
    composition root in
    ``tests/integration/test_st05_supertrend_buy_supervisor_composition.py``."""
...
def test_the_daily_cap_resolves_to_thirty_thousand_rupees(engine_config):
    """Spec section 12: 3% of a Rs 10,00,000 reference capital. Independent of
    ``st12_supertrend_buy``'s own cap — no combined cap exists."""
...
def test_the_lookback_budget_covers_the_seventy_five_bucket_floor(engine_config):
    ...
    committed budget must be at least that. Unchanged from ``st12_supertrend_buy``
    — this floor is not derived from the multiplier."""
    session = MarketSession(
        SessionConfig(
...
def test_the_holiday_calendar_matches_st12_supertrend_buys_line_for_line(engine_config):
    """Spec section 7.2: the three calendars (global, st12, st05) are deliberately
    duplicated so they stay line-for-line comparable."""
    st12 = load_resolved_config(CONFIG_ROOT, RUNTIME_ID, "st12_supertrend_buy")
    assert tuple(st12.strategy.parameters["holidays"]) == engine_config.holidays
...
def test_paper_execution_uses_the_current_model_not_the_legacy_zero_slippage(worker):
    """Spec section 13, the same intentional architecture-level deviation from the
    legacy simulator ``st12_supertrend_buy``/``c921_ema_cross_buy`` already record:
    legacy ran zero slippage and no latency. This is the same canonical intraday
    paper block, so results stay comparable across all three single-leg intraday
    strategies."""
