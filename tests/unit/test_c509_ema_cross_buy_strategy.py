"""Behaviour-level proofs for ``EmaCross5x9BuyStrategy`` — the second real
...
the *defaults* really are 5 and 9. Every numeric candle sequence here was
...
def _close_trade(strategy: EmaCross5x9BuyStrategy, position: OpenPosition, exit_price: float,
...
def test_get_strategy_by_name_builds_from_cfg_parameters():
    ...
    assert strategy._ema_slow.period == 9


# ------------------------------------------------------------------ 2. EMA periods
def test_default_periods_are_5_and_9():
...
def test_ema_slow_must_be_greater_than_ema_fast():
    with pytest.raises(ValueError, match="ema_slow"):
        EmaCross5x9BuyStrategy(ema_fast=9, ema_slow=5)
    with pytest.raises(ValueError, match="ema_slow"):
        EmaCross5x9BuyStrategy(ema_fast=9, ema_slow=9)
