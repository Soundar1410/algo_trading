"""Behaviour-level proofs for ``SupertrendBuy1x0p5Strategy``.

Full spec: ``strategies/intraday_options/st05_supertrend_buy/
SUPERTREND_BUY_1_0P5_ALGO_TRADING_SPEC.md``. Parity source: the in-repo
``st12_supertrend_buy`` strategy — **not** an independent legacy port, and
**not** ``NiftyFixedStrikeSuperTrend_Master_Specification.md``, which documents
a different fixed-strike, two-position strategy (settled before implementation,
inherited from ``st12_supertrend_buy``'s own review).
...
Every underlying-candle sequence below was walked through the real
:class:`~common.indicators.supertrend.SuperTrend` at multiplier ``0.5`` before being
hard-coded — **not** copied from ``st12_supertrend_buy``'s own numerics and assumed to
still flip the same way. They happen to produce identical flip patterns to the 1.2
variant on every sequence carried over (verified directly, not assumed), because these
sequences use large gap moves where the carried-forward band dominates the tighter
0.5 width too. The one sequence-shaped test that does NOT carry over unchanged is
``test_the_first_candle_always_seeds_uptrend_at_this_multiplier``'s 1.2 original,
replaced below by a test that states the real (boundary-sensitive) 0.5 seed rule.
...
def _strategy(*, trusted: bool = True, **kwargs) -> SupertrendBuy1x0p5Strategy:
    ...
    """
    strategy = SupertrendBuy1x0p5Strategy(**kwargs)
    strategy.on_warmup_complete(context_trusted=trusted)
    return strategy


def _feed(strategy: SupertrendBuy1x0p5Strategy, closes: list[float]) -> list[object]:
...
# --------------------------------------------------------------- registration
def test_strategy_registers_as_st05_supertrend_buy():
    assert "st05_supertrend_buy" in available_strategies()
    assert SupertrendBuy1x0p5Strategy.name == "st05_supertrend_buy"
    assert issubclass(SupertrendBuy1x0p5Strategy, BaseStrategy)
...
def test_get_strategy_by_name_builds_from_cfg_parameters():
    ...
    strategy = get_strategy("st05_supertrend_buy", cfg)
    assert isinstance(strategy, SupertrendBuy1x0p5Strategy)
...
def test_explicit_kwargs_win_over_cfg_parameters():
    ...
    assert SupertrendBuy1x0p5Strategy(cfg, lots_per_trade=7).quantity_lots == 7


# ------------------------------------------------------------------ parameters
def test_shipped_defaults_match_st12_supertrend_buy_except_the_multiplier():
    """Spec sections 6/8/10: ATR period 1 (unchanged), multiplier 0.5 (the one
    delta), 10 lots, 8% trail with a 4% activation gate — identical to
    ``st12_supertrend_buy``'s own shipped defaults otherwise."""
    strategy = SupertrendBuy1x0p5Strategy()
    assert strategy._supertrend.period == 1
    assert strategy._supertrend.multiplier == 0.5
...
def test_default_warmup_min_bars_is_a_full_session_not_the_atr_period():
    ...
    raises the floor locally; the indicator is untouched. Identical to
    ``st12_supertrend_buy``: 75 is not recomputed for this multiplier."""
    strategy = SupertrendBuy1x0p5Strategy()
...
def test_warmup_spec_never_lowers_the_indicators_own_floor():
    ...
    strategy = SupertrendBuy1x0p5Strategy(supertrend_period=200)
    spec = strategy.warmup_spec()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"supertrend_multiplier": -0.5}, "supertrend_multiplier must be > 0"),
...
def test_invalid_parameters_are_refused_at_construction(kwargs, message):
    with pytest.raises(ValueError, match=message):
        SupertrendBuy1x0p5Strategy(**kwargs)


def test_supertrend_period_below_one_is_refused_by_the_indicator():
    with pytest.raises(ValueError, match="period must be >= 1"):
        SupertrendBuy1x0p5Strategy(supertrend_period=0)
...
def test_the_seeding_candle_never_produces_an_entry():
    ...
    Holds regardless of which direction the seed lands on (see the boundary test
    below) — this is the actual "no entry on the initial state" guarantee."""
    strategy = _strategy()
    assert strategy.on_candle(_candle(20000), _ts(0)) is None
    assert strategy._supertrend.state.flipped is False


def test_the_seed_direction_at_multiplier_0p5_is_boundary_sensitive():
    """Why this variant's seed-direction reasoning is NOT ``st12_supertrend_buy``'s
    "always UP" claim, re-derived against the real indicator rather than assumed.

    At multiplier 1.2, ``lower_basic = hl2 - 1.2(h-l)`` sits strictly below ``l`` for
    every candle, so bar 0 always seeds UP. At exactly 0.5 that inequality is no
    longer strict: ``hl2 - 0.5(h-l)`` reduces algebraically to exactly ``l``, so
    whether the seed comes out UP (``close >= lower_basic``) or DOWN
    (``close < lower_basic``) depends on IEEE-754 floating-point rounding of that
    subtraction against the candle's own low. A "nice" round-number candle rounds
    to an exact match and seeds UP; an ordinary fractional candle can round the
    other way and seed DOWN — both are correct behaviour of the shared indicator,
    not a strategy defect.

    Either way, spec section 6's "first initialization/seed produces no entry" rule
    holds regardless of which direction the seed lands on — proven independently by
    ``test_the_seeding_candle_never_produces_an_entry`` above, which is not
    conditioned on the seed direction at all.
    """
    # A round-number candle: hl2 - 0.5(h-l) resolves to exactly `l`, `close == low`
    # satisfies `close >= lower_basic`, seeds UP.
    round_number_strategy = _strategy()
    round_number_candle = OHLC(high=20100, low=19900, close=19900)
    assert round_number_strategy.on_candle(round_number_candle, _ts(0)) is None
    assert round_number_strategy._supertrend.state.trend == UPTREND

    # A fractional candle closing at its own low: verified directly against the real
    # indicator to round the other way and seed DOWN. Not hand-picked to "look like"
    # a boundary case — it is the actual output of SuperTrend(1, 0.5) on this input.
    fractional_strategy = _strategy()
    fractional_candle = OHLC(high=19921.11, low=19739.91, close=19739.91)
    assert fractional_strategy.on_candle(fractional_candle, _ts(0)) is None
    assert fractional_strategy._supertrend.state.trend == DOWNTREND
    # The identical candle at st12_supertrend_buy's multiplier (1.2) seeds UP —
    # proving this is a property of the 0.5 band width, not of the candle itself.
    from common.indicators.supertrend import SuperTrend

    control = SuperTrend(period=1, multiplier=1.2)
    control_state = control.update(fractional_candle)
    assert control_state.trend == UPTREND

    # And a candle whose close sits strictly above its own low still seeds UP,
    # exactly as at 1.2 — the boundary only bites exactly at the low.
    ordinary_strategy = _strategy()
    ordinary_candle = OHLC(high=20100, low=19900, close=19901)
    assert ordinary_strategy.on_candle(ordinary_candle, _ts(0)) is None
    assert ordinary_strategy._supertrend.state.trend == UPTREND


def test_trusted_up_context_fresh_down_flip_buys_pe():
    """Spec 18.1: "Trusted context UP, fresh flip DOWN | Buy ATM weekly PE". Sequence
    re-verified against the real SuperTrend(1, 0.5): identical flip to the 1.2
    variant on this input."""
    strategy = _strategy()
...
def test_trusted_down_context_fresh_up_flip_buys_ce():
    ...
    Sequence re-verified against the real SuperTrend(1, 0.5): identical flip to the
    1.2 variant on this input (which is itself the legacy suite's own
    ``test_flip_up_enters_ce`` sequence)."""
    strategy = _strategy()
...
def test_an_up_context_continuation_produces_no_entry():
    """Spec 18.1: "UP remains UP | No new order". Re-verified: no flip at 0.5 either."""
    strategy = _strategy()
...
def test_day_start_reset_clears_indicator_trust_and_exit_state():
    ...
    manufactures a false flip. Matches ``st12_supertrend_buy``'s own ``reset()``."""
    strategy = _strategy()
...
def test_status_is_safe_before_the_first_candle():
    assert "none" in SupertrendBuy1x0p5Strategy().status()


# ------------------------------------------------------------ 18.4 premium exit
def test_needs_option_candles_is_true():
    assert SupertrendBuy1x0p5Strategy().needs_option_candles is True
...
def test_option_selection_is_atm_zero_steps():
    selection = SupertrendBuy1x0p5Strategy().option_selection
    assert selection.moneyness is Moneyness.ATM
    assert selection.steps == 0


def test_trade_side_is_always_buy():
    assert SupertrendBuy1x0p5Strategy().trade_side is OrderSide.BUY


def test_risk_manager_is_hard_stop_and_disabled_by_default():
    """Same choice ``st12_supertrend_buy`` made: ``hard_stop`` with its own disabled
    default is the behavioural equivalent of the legacy config's
    ``sl_lock_trail`` with all four thresholds null — a per-tick rupee rule that
    never fires."""
    strategy = SupertrendBuy1x0p5Strategy()
...
def test_the_strategy_never_names_a_lot_size():
    ...
    source = __import__(
        "inspect"
    ).getsource(SupertrendBuy1x0p5Strategy)
