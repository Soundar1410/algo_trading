...
def test_the_contract_was_chosen_at_runtime_and_asked_for_upstream(worker_config):
    ...
    Entries are ``(security_id, segment, mode)`` as of Phase 4 Part 5 (Part 1 added
    the segment). A **simulated** contract has neither a real exchange segment nor a
    real order book, so ``None`` twice is the correct value and not an omission —
    the assertion below pins that rather than ignoring it.
    """
    ...
    assert (CE_CONTRACT, None, None) in requested, (
        "the runtime subscription never reached the supervisor"
    )
