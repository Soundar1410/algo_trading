...
exit behaviour for ``st12_supertrend_buy`` — through a real ``TradingEngine`` over a
real ``ExecutionRepository`` on a temporary database.

Companion to ``test_st12_supertrend_buy_recovery.py``; the shared harness and the
reason the engine is assembled rather than run through ``run_worker`` are documented
in ``_st12_supertrend_buy_fixtures.py``.
...
import pytest
from _st12_supertrend_buy_fixtures import (
...
ENTRY_CORRELATION = "p_io_st12_20260820_0001"
CLOSE_CORRELATION = "p_io_st12_20260820_0002"
SQUARE_OFF_CORRELATION = "p_io_st12_20260820_0003"
