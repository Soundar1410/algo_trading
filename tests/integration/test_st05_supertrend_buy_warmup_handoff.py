"""Warm-up handoff for ``st05_supertrend_buy``, through a real ``TradingEngine``.

The unit-level companion (``tests/unit/test_st05_supertrend_buy_warmup.py``) proves
...
Every number below was walked through the real ``SuperTrend(1, 0.5)`` before being
hard-coded — **not** copied from ``st12_supertrend_buy``'s own 1.2 numerics: the
warm-up series contains **ten** genuine flips at this multiplier too (re-verified,
not assumed) and leaves the indicator in an uptrend with its active band at
**24185.0** (1.2's own band was 24178.0 on the identical series — the tighter
multiplier does shift this particular value), so a live close of **24235.0**
continues that trend and one of **24135.0** flips it.
"""
...
#: The active SuperTrend(1, 0.5) band after the warm-up series below — verified
#: against the real indicator, not derived from st12_supertrend_buy's own 24178.0.
WARMED_BAND = 24185.0
CONTINUATION_CLOSE = WARMED_BAND + 50.0  # 24235.0 — stays UP
FLIP_CLOSE = WARMED_BAND - 50.0  # 24135.0 — flips DOWN
#: round(24135 / 50) * 50 == 24150 — the ATM strike the flip resolves to (same
#: strike st12_supertrend_buy's own 24128 close resolves to, coincidentally).
...
def _build_engine(
    ...
    *,
    clock_at: datetime,
) -> tuple[TradingEngine, SupertrendBuy1x0p5Strategy, PositionManager, _CountingGateway]:
